...
void generateMoves()
{
	...
	for(int i=0;i<11;i++){
		for(int j=0;j<11;j++){
			if(i>1 && inRange(myRow+i, myCol-5+j) && world[myRow+i][myCol-5+j]=='*' && abs(i)+abs(j-5)<7){
				if(inRange(myRow+1,myCol+1)){
					if(j>5 && canDigRight){
						...
						while(inRange(myRow+k,myCol+1) && (world[myRow+k][myCol+1]=='.' ||world[myRow+k][myCol+1]=='*'))
							k++;
						int f=0;
						for(int i=-7+k;i<7-k;i++){
							if(inRange(myRow+k-1,myCol+1+i) && enemyPresent[myRow+k-1][myCol+1+i]>=0)
								f++;
						}
						if(f==0){
							goldDig=RIGHT;
				...
				if(inRange(myRow+1,myCol-1)){
					if(j<=5 && canDigLeft){
						...
						int f=0;
						for(int i=-7+k;i<7-k;i++){
							if(inRange(myRow+k-1,myCol+1+k) && enemyPresent[myRow+k-1][myCol-1+k]>=0)
								f++;
						}
						if(f==0){
							goldDig=LEFT;
	...
	if((goldDig==LEFT || goldDig==RIGHT) && canDig){
	...
	if (threat[TOP] && !threat[BOTTOM] && canMove[BOTTOM]){
		move="BOTTOM";
		return;
	}
	else if(threat[BOTTOM] && !threat[TOP] && canMove[TOP]){
		move="TOP";
		return;
	}
