...
int shortestPath(int sx,int sy,char target,int num)
{
	...
	while(begin<end){
		...
		if(world[qux[begin]][quy[begin]]==target){
			if(target=='.'){
				if(inRange(qux[begin]+2,quy[begin]+1)){
		...
		//move up
		if(inRange(qux[begin]-1,quy[begin]) && !visited[qux[begin]-1][quy[begin]]&& 
		  world[qux[begin]-1][quy[begin]]!='='&& world[qux[begin]][quy[begin]]=='H'){
			qux[end]=qux[begin]-1;
			quy[end]=quy[begin];
			parent[end]=begin;
			end++;
...
{  
	...
	for ( int turnNumber = 0; turnNumber < turns; turnNumber++ ) {
		...
		if(myRow >= 0 && myCol >= 0 ){
			...
			if(myRow==prevmyRow && myCol==prevmyCol)
				myDirection=5;
			for(int i=0; i < enemyCount; i++ ){ 	
